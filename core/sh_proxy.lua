...
-- Silent existence check, no warning on miss — for callers that poll while waiting on a
-- component to boot (e.g. plsr.X's self-gating retry loop in sh_pulsar.lua). FetchComponent
-- logs on every miss because a one-shot fetch failing is worth flagging; a poll loop retrying
-- 100 times while a dependency is still starting is not.
exports('HasComponent', function(component)
