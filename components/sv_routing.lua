...
COMPONENTS.Routing = {
    ...
    GetPlayerRoute = function(self, source)
        return {
            state = COMPONENTS.State:Player(source).currentRoute,
            oldRoute = COMPONENTS.State:Player(source).oldRoute,
            route = GetPlayerRoutingBucket(source)
        }
    end,
    AddPlayerToRoute = function(self, source, route, force)
        local plr = COMPONENTS.State:Player(source)
        if plr.currentRoute == route and not force then
        ...
        plr.oldRoute = currentRoute
        SetPlayerRoutingBucket(source, route)
        plr.currentRoute = route
